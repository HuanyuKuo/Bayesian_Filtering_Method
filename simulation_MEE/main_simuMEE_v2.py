...
simu_NL = 1*10**5  # Totla number of lineage having data generated by this program
meanfitness = 0.0 # initial meanfitness
epsilon = mc.epsilon
T=10 # total time points
NL_s = 2000 # number of adaptive lineages
...
'''
Rt = [10**6]*T
cycles = [mc.cycle for _ in range(T)]
const = Constant(Rt, cycles)
glob = Global(const)
current_timestep =0
...
if __name__ == '__main__':
    ...
    lineage_info =  {'lineage_name': '20220226'}
    ...
    cycles = []
    cycles.extend( [sum(const.Ct[1:i]) for i in range(1,len(const.Ct))])
